...
def main():
    ...
    goal = [cx[-1], cy[-1]]  # 마지막 점을 목표로 설정

    # 스플라인 경로 계산
    cx, cy, cyaw, ck, s = cubic_spline_planner.calc_spline_course(cx, cy, ds=0.1)
    target_speed = 10.0 / 3.6  # 목표 속도 (km/h -> m/s 변환)

    sp = calc_speed_profile(cx, cy, cyaw, target_speed)

    # LQR 제어를 통한 경로 추종 예측 실행
    ...
    if show_animation:  # pragma: no cover
        plt.close()
        plt.subplots(1)
        plt.plot(cx, cy, "-r", label="input_spline")  # 직사각형 스플라인 경로
        plt.plot(x, y, "-g", label="tracking course")  # 추적 경로
        plt.grid(True)
        plt.axis("equal")
        plt.xlabel("x[m]")
        plt.ylabel("y[m]")
